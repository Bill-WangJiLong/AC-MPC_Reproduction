...
class BaseAlgorithm(ABC):
    ...
    def _setup_learn(
        ...
        if reset_num_timesteps or self._last_obs is None:
            reset_result = self.env.reset()  # pytype: disable=annotation-type-mismatch
            if isinstance(reset_result, tuple) and len(reset_result) == 2:
                self._last_obs, self._last_state = reset_result
            else:
                self._last_obs = reset_result
                if hasattr(self.env, "get_state"):
                    self._last_state = self.env.get_state()
                else:
                    self._last_state = np.asarray(self.env.env_method("get_state"), dtype=np.float32)
