...
bool VecEnv<EnvBase>::getState(Ref<MatrixRowMajor<>> state) {
  if (state_dim_ <= 0) {
    logger_.error("The environment does not expose a state vector.");
    return false;
  }
  if (state.rows() != num_envs_ || state.cols() != state_dim_) {
    logger_.error(
      "Input state matrix dimensions do not match with that of the environment.");
    return false;
  }
  for (int i = 0; i < num_envs_; i++) {
    if (!envs_[i]->getState(state.row(i))) return false;
  }
  return true;
}
...
#else
  if (render) {
    logger_.warn("Unity bridge support is disabled in this headless build.");
  }
  unity_render_ = false;
  return !render;
#endif
