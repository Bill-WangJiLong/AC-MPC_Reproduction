...
PYBIND11_MODULE(flightgym, m) {
  ...
  py::class_<VecEnv<RacingEnv>>(m, "RacingEnv_v1")
    ...
    .def(py::init<const std::string&>())
    .def(py::init<const std::string&, const bool>())
    .def("reset", &VecEnv<RacingEnv>::reset)
    .def("step", &VecEnv<RacingEnv>::step)
    .def("testStep", &VecEnv<RacingEnv>::testStep)
    .def("setSeed", &VecEnv<RacingEnv>::setSeed)
    .def("close", &VecEnv<RacingEnv>::close)
    .def("isTerminalState", &VecEnv<RacingEnv>::isTerminalState)
    .def("curriculumUpdate", &VecEnv<RacingEnv>::curriculumUpdate)
    .def("connectUnity", &VecEnv<RacingEnv>::connectUnity)
    .def("disconnectUnity", &VecEnv<RacingEnv>::disconnectUnity)
    .def("getNumOfEnvs", &VecEnv<RacingEnv>::getNumOfEnvs)
    .def("getObsDim", &VecEnv<RacingEnv>::getObsDim)
    .def("getActDim", &VecEnv<RacingEnv>::getActDim)
    .def("getStateDim", &VecEnv<RacingEnv>::getStateDim)
    .def("getState", &VecEnv<RacingEnv>::getState)
    .def("getExtraInfoNames", &VecEnv<RacingEnv>::getExtraInfoNames)
    .def("__repr__", [](const VecEnv<RacingEnv>& a) {
      return "AC-MPC Flightmare Racing Environment";
    });
